...
def _format_markdown_entry(rec: Dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append("### Biological Markers & Context")
    lines.append(f"**Cell Identity Signatures**: {rec.get('CellIdentitySignatures', 'None reported')}")
    lines.append(f"**Perturbations**: {rec.get('PerturbationsUsed', 'None')}")
    lines.append("")
